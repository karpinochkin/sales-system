...
void QUnAccountManager::initDB()
{
    if(dataBase->driverName()=="QMYSQL")
    {
        ...
        if (dataBase->open())
        {
            ...
            dataBase->close();
        } else qDebug()<<"QUnAccountManager Error open database: "<<dataBase->lastError().text();
    }
...
QSqlQuery *QUnAccountManager::execQuery(QString textQuery, bool *ok)
{
    QSqlQuery *Output;
...
bool QUnAccountManager::createAccount (QUNAccountInfo accountInfo)
{
    bool Output = false;
...
//    {
//        qDebug()<<"open";
//        QSqlQuery * queryCheckLogin1 = new QSqlQuery(*dataBase);
//        if (queryCheckLogin1->next())
//        {
//            qDebug()<<"Query:"<<queryCheckLogin1;
//        }
//    }
//    else
//    {
...
bool QUnAccountManager::editAccount(QUNAccountInfo &accountInfo)
{
    bool Output = false;
    ...
    QString textQuery = "update UNusers set "
                        ...
                        "UNregdate = '" + accountInfo.regdate.toString("yyyy-MM-dd hh:mm:ss") + "',"
                        ...
                        "UNnumbercode = '" + QString::number(accountInfo.numbercode) + "' "
                        "where id_UN = '" + QString::number(accountInfo.id) + "';";

    QSqlQuery *queryEditAccount = execQuery(textQuery,&Output);

    if (!Output) {
        qDebug()<<"Error edit account:"<<queryEditAccount->lastError().text();
        qDebug()<<"textQuery = "<<textQuery;
    }

    delete queryEditAccount;
...
QVector<QUNAccountInfo> QUnAccountManager::getAccountsList()
{
    QVector<QUNAccountInfo> Output;
...
QUNAccountInfo QUnAccountManager::getAccountInfo(int id)
{
...
QUNAccountInfo QUnAccountManager::getAccountInfo(QString login)
{
    QUNAccountInfo Output;
    ...
    if (dataBase->open())
    {
        ...
        if (queryAccountInfo->next())
        {
            ...
            Output.visible = queryAccountInfo->value(9).toBool();
            Output.rights = queryAccountInfo->value(10).toInt();
            Output.regdate = queryAccountInfo->value(11).toDateTime();
            Output.activate = queryAccountInfo->value(12).toBool();
            Output.changepassword = queryAccountInfo->value(13).toBool();
            Output.datepassword = queryAccountInfo->value(14).toDateTime();
            Output.activemail  = queryAccountInfo->value(15).toBool();
            Output.activenumber  = queryAccountInfo->value(16).toBool();
            Output.emailcode = queryAccountInfo->value(17).toInt();
            Output.numbercode = queryAccountInfo->value(18).toInt();
...
bool QUnAccountManager::updateAccount (int id, QUNAccountInfo accountInfo)
{
    bool Output = false;
