...
QUNAccountInfo QUnNetProtokolCLient::getCurrentAccountInfo()
{
...
void QUnNetProtokolCLient::setNewPassword(QString login, QString Password)
{
    QByteArray Output;
...
void QUnNetProtokolCLient::slotSocketDisconnected()
{
    ...
    if (flagClearOnDisconnect) {
//        qDebug()<<"QUnNetProtokolCLient::clear packet manager";
        packetManager->clear();
    }
}
    ...
    if (countPacket == 0)
    {
        ...
        if (*(ptrCommand+1) == 0)
        {
            ...
            case 0x0005: command05(getPacket(command,idBusiness));
                break;
            case 0x0006: command06(getPacket(command,idBusiness));
                break;
            }
...
void QUnNetProtokolCLient::command06 (QByteArray data)
{
    ...
    emit signalResultSetNewPassword(result,login);
}
