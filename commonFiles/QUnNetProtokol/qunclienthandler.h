...
public:
    ...
    QUNAccountInfo *getCurrentAccountInfo();
    int getMaxLengthPacket();

    QUnPacketManager *getPacketManager();
    QUNAccountInfo *getQUNAccountInfo();

private:
    bool Authorized;
    QUNAccountInfo currentAccount;
    QTimer *timerPing;
    ...
    void command01(QByteArray data); //Получение ping
    ...
    void sendCommand04 (quint16 command,UnErr err); //Отправка сообщения о ошибке

    void command05 (QByteArray data); //Запрос данных текущего пользователя
    void sendCommand05 (QUNAccountInfo userInfo);

    void command06(QByteArray data); //Смена пароля текущего пользователя

    //Команды администратора
    void command30(QByteArray data); //Запрос списка пользователей на сервере
    void command31(QByteArray data); //Редактирование аккаунта
    void command32(QByteArray data); //Добавить пользователя
