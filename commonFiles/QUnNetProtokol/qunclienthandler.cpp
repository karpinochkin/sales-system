...
QUNAccountInfo *QUnClientHandler::getCurrentAccountInfo()
{
    ...
    if (countPacket == 0)
    {
        ...
        if (*(ptrCommand+1) == 0)
        {
            ...
            case 0x0005: command05(getPacket(command));
                break;
            case 0x0006: command06(getPacket(command));
                break;


                //Команды администратора
            case 0x0030: command30(getPacket(command));
                break;
            case 0x0031: command31(getPacket(command));
                break;
            case 0x0032: command32(getPacket(command));
                break;
            }
...
void QUnClientHandler::command05 (QByteArray data)
{
    ...
    if (Authorized)
    {
        //qDebug()<<currentAccount.toString();
        sendCommand05(currentAccount);

    } else sendCommand04(0x0005,UnErr::errNoAuthorized);
}

void QUnClientHandler::sendCommand05 (QUNAccountInfo userInfo)
{
    QByteArray buffer;
//    quint8 null_ =0;
...
void QUnClientHandler::command06(QByteArray data)
{
    ...
    bool result;
    if (currentAccount.login == login) {
        currentAccount.password = password;
        result = accountManager->editAccount(currentAccount);
    } else {
        if (currentAccount.rights == 0) {
            currentAccount.password = password;
            result = accountManager->editAccount(currentAccount);
        } else {
            result = false;
    ...
    sendData(0x00,0x06,0,Output);
}

void QUnClientHandler::command30(QByteArray data)
{
    Q_UNUSED(data)

    if (currentAccount.rights <= 0) {
         QVector<QUNAccountInfo> users = accountManager->getAccountsList();

         QByteArray Output;
         QDataStream streamOut (&Output,QIODevice::ReadWrite);

         streamOut<<(quint16)users.length();
         QUNAccountInfo currentAccountInfo;
         for (int i=0;i<users.length();i++) {
             currentAccountInfo = users.at(i);

             if ((currentAccount.rights > currentAccountInfo.rights) &&
                     (currentAccount.id != currentAccountInfo.id))
                            currentAccountInfo.password.clear();

             currentAccountInfo>>streamOut;
         }

        sendData(0x00,0x30,0,Output);

    } else {
        sendCommand04(0x30,errNoRights);
    }
}

void QUnClientHandler::command31(QByteArray data)
{
    QDataStream streamIn (&data,QIODevice::ReadOnly);

    QUNAccountInfo currentAccountInfo;
    currentAccountInfo<<streamIn;

    bool result;
    if ((currentAccount.rights > currentAccountInfo.rights) &&
            (currentAccount.id != currentAccountInfo.id)) {

        result = accountManager->editAccount(currentAccountInfo);
        result = true;
    } else {
        sendCommand04(0x31,errNoRights);
        result = false;
    }
    ...
    sendData(0x00,0x31,0,Output);
}

void QUnClientHandler::command32(QByteArray data)
{
    QDataStream streamIn (&data,QIODevice::ReadOnly);
    ...
    if (currentAccount.rights > currentAccountInfo.rights)
    {
        if (currentAccountInfo.rights < currentAccount.rights)
            currentAccountInfo.rights = currentAccount.rights;

        result = accountManager->createAccount(currentAccountInfo);

    } else {
        sendCommand04(0x32,errNoRights);
        result = false;
...
QUNAccountInfo *QUnClientHandler::getQUNAccountInfo()
{
    return &currentAccount;
}
