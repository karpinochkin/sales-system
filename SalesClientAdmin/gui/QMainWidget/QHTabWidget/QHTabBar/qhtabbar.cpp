...
QSize QHTabBar::tabSizeHint(int index) const
{
    ...
    A.setWidth(fm.horizontalAdvance(this->tabText(index))
               + this->iconSize().width()
               + fm.horizontalAdvance(this->tabText(index)) / 3 + 35);

    if (fm.height()>this->iconSize().height())
    {
        A.setHeight(static_cast<int>(fm.height()*1.5));
    } else A.setHeight(static_cast<int>(this->iconSize().height()*1.5));
...
void QHTabBar::paintEvent(QPaintEvent *event)
{
    ...
    for (int index = 0; index < count(); index++)
    {
        ...
        QFontMetrics fm(this->font());
        int w = fm.horizontalAdvance(tempText);
        int h = fm.height();
        int x = tabrect.left()
                + this->iconSize().width()
        ...
        tempIcon.paint(&painter, 10, tabrect.center().y()-iconSize().height()/2,
                       tab.iconSize.width(), tab.iconSize.height(),
